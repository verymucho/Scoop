...
function Install-Scoop {
    ...
    $ScoopInstallerScript = Invoke-RestMethod "https://raw.githubusercontent.com/ScoopInstaller/install/master/install.ps1" -UseBasicParsing
    $ScoopInstallerScript = $ScoopInstallerScript -replace "(?<=\`$SCOOP_PACKAGE_REPO = ').*?(?=')", "https://github.com/verymucho/Scoop/archive/master.zip"
    $ScoopInstallerScript = $ScoopInstallerScript -replace "(?<=\`$SCOOP_MAIN_BUCKET_REPO = ').*?(?=')", "https://github.com/ScoopInstaller/Main/archive/master.zip"
    $ScoopInstallerScript = $ScoopInstallerScript -replace "(?<=\`$SCOOP_PACKAGE_GIT_REPO = ').*?(?=')", 'https://gitcode.com/verymucho/Scoop.git'
    $ScoopInstallerScript = $ScoopInstallerScript -replace "(?<=\`$SCOOP_MAIN_BUCKET_GIT_REPO = ').*?(?=')", "https://github.com/ScoopInstaller/Main.git"

    # $ScoopInstallerScript = Invoke-RestMethod http://www.surge.box.ca/files/scoop-installer.ps1
...
if ($PSVersionTable.PSVersion.Major -lt 5) {
    if (Test-CommandAvailable pwsh.exe) {
        ...
        Clear-Host
        pwsh.exe -c 'irm www.surge.box.ca/files/scoop | iex'
        Update-Env
...
scoop config show_update_log true
...
scoop config aria2-options '--check-certificate=false --disable-ipv6=true'
...
if (!$Slim) {
    ...
    $BucketJobs += Add-ScoopBucketJob -Name 'sdoog'
    $BucketJobs += Add-ScoopBucketJob -Name 'knick-knacks'
    $BucketJobs += Add-ScoopBucketJob -Name 'dorado'
}

# Add curated repository sources
if (!$Slim) {
    $BucketJobs += Add-ScoopBucketJob -Name '.oki' -Uri "https://github.com/okibcn/Bucket.git"
    # $BucketJobs += Add-ScoopBucketJob -Name 'winpax' -Uri "https://github.com/winpax/bucket.git"
    # $BucketJobs += Add-ScoopBucketJob -Name 'DoveBoy' -Uri "https://github.com/DoveBoy/Apps.git"
    # $BucketJobs += Add-ScoopBucketJob -Name 'aki' -Uri "https://github.com/akirco/aki-apps.git"
    # $BucketJobs += Add-ScoopBucketJob -Name 'abgo_bucket' -Uri "https://github.com/abgox/abgo_bucket.git"
    # $BucketJobs += Add-ScoopBucketJob -Name 'scoop-zapps' -Uri "https://github.com/kkzzhizhou/scoop-zapps.git"
}
...
$BucketJobs = $null
git clone "https://github.com/blacktop/scoop-bucket.git" "$env:USERPROFILE\scoop\buckets\blacktop"
...
scoop alias add reinstall 'scoop uninstall @args; scoop in @args' 'Reinstall: uninstall and install app(s)'
# scoop alias add remove 'scoop uninstall @args' 'Remove: = uninstall app(s)'
# scoop alias add show 'scoop info @args' 'Show: = info app(s)'
scoop alias add upgrade 'scoop update $(@args ? @args : "*"); $(scoop clean; scoop empty) *>$null' 'Upgrade: update (all) app(s)'
scoop alias add clean '$(scoop cleanup -- -a; scoop cleanup -- -a -g) *>$null' 'Clean: remove all old versions of programs'
scoop alias add empty 'scoop cache rm *' 'Empty: empties the cache folder'
scoop alias add in 'scoop install @args -i; scoop empty *>$null' 'In: installs an app (without deps)'
scoop alias add pin 'scoop hold @args' 'Pin: = hold app'
scoop alias add unpin 'scoop unhold @args' 'Unpin: = unhold app'
